...
namespace Wave.Essence.Controller
{
	...
	{
		private static string LOG_TAG = "ShowIndicator";
		...
		void Start()
		{
			auto = this.GetComponent<AutoLayout>();
			if (auto != null)
			{
				auto.enabled = false;
			}
		...
		public void createIndicator()
		{
			...
			{
				PrintInfoLog("TextInd is not found!");
				return;
			}
			else
			{
				PrintDebugLog("TextInd is found!");
			}
			...
			if (linePrefab == null)
			{
				PrintInfoLog("LineInd is not found!");
				return;
			}
			else
			{
				PrintDebugLog("LineInd is found!");
			}

			if (_HMD == null)
				_HMD = Camera.main.gameObject;
		...
		public void createAutoIndicator()
		{
			if (auto == null)
			{
				...
				if (auto == null)
				{
					this.gameObject.AddComponent<AutoLayout>();
					auto = this.GetComponent<AutoLayout>();
					if (auto == null)
						Log.w(LOG_TAG, "AutoLayout can't be found.", true);
				}
			}
